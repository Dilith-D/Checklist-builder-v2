...
const generateZuperPayload = (checklist, config) => {
  try {
    ...
    const { categoryUid, statusUid } = config;
     ...
     // Map checklist items to Zuper format 
...
console.log(getMetaOptionsByComponent(componentType));
  ...
  return {
    ...
    console.log(`✅ Generated Zuper payload with ${zuperChecklist.length} items`);
...
const getZuperCategories = async (apiKey, region) => {
  try {
    const url = `https://${region}.zuperpro.com/api/settings/job_categories`;
 ...
 */
const getZuperStatuses = async (apiKey, region) => {
  try {
    const url = `https://${region}.zuperpro.com/api/settings/job_status`;
