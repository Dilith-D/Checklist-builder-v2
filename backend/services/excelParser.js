  ...
  try {
    ...
    const validation = {
      ...
      metadata: {
        ...
        hasHeader: false,
        expectedFormat: 'question|type|option|required'
      }
    };
    ...
    if (headerRow && headerRow.length >= 4) {
      ...
      const expectedHeader = ['question', 'type', 'option', 'required'];
      ...
      if (headerMatches) {
        ...
        validation.warnings.push('Header row may not match expected format: question|type|option|required');
      }
    } else {
